...
type ListenVisibility = 'sync' | 'async' | 'query'
type ListenEventName = 'mutation' | 'welcome' | 'reconnect'
type MutationSelection = {query: string} | {id: string}
type SanityReference = {_ref: string}
type AssetMetadataType = 'location' | 'exif' | 'image' | 'palette' | 'lqip' | 'none'
...
type ClientConfig = ProjectClientConfig | ProjectlessClientConfig
...
type PreviousNextListenOptions = ListenOptions & {
  includeResult: true
  includePreviousRevision: true
}

type PreviousListenOptions = ListenOptions & {
  includePreviousRevision: true
  includeResult: false
}

type NextListenOptions = ListenOptions & {
  includePreviousRevision: false
  includeResult: true
}

type ResultlessListenOptions = ListenOptions & {
  includeResult: false
  includePreviousRevision: false
}

type FilteredResponseQueryOptions = RequestOptions & {
  filterResponse: true
}

type UnfilteredReponseQueryOptions = RequestOptions & {
  filterResponse: false
}

type QueryOptions = FilteredResponseQueryOptions | UnfilteredReponseQueryOptions
